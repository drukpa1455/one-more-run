...
def parser() -> argparse.ArgumentParser:
    ...
    run_command.add_argument(
        "--hindsight", metavar="BANK", help="enable a Hindsight memory bank"
    )

    akash_command = commands.add_parser(
        "akash", help="deploy, run, and close an Akash GPU worker"
    )
    ...
    akash_command.add_argument(
        "--yes", action="store_true", help="authorize the displayed spend limits"
    )
...
def run(args: argparse.Namespace) -> int:
    ...
    memory_environment = os.environ
    if bank := getattr(args, "hindsight", None):
        memory_environment = {**os.environ, "OMR_HINDSIGHT_BANK": bank}
    memory = hindsight_from_environment(memory_environment)
